...
(function ($, window, undefined)
{
    ...
    function loadData()
    {
        ...
        if ( that.initRequest ) {
            request = that.initRequest;
            $('input.search-field').val(request.searchPhrase);
            this.sort = request.sort;
            that.initRequest = null;
        ...
        if(window.sessionStorage){
            var fileter_name = 'bootgrid_filter_' + $.md5(url);
            window.sessionStorage.setItem(fileter_name, JSON.stringify(request));
        }
    ...
    var Grid = function(element, options)
    {
        ...
        if(window.sessionStorage){
            var fileter_name = 'bootgrid_filter_' + $.md5(this.options.url);
            var json = window.sessionStorage.getItem(fileter_name);
            if ( json ) {
                this.initRequest = JSON.parse(json);
            }
        }
    ...
    Grid.defaults = {
        ...
        rowCount: [15, 30, 50, -1], // rows per page int or array of int (-1 represents "All")
