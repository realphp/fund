...
<form class="form-horizontal tpx-ajaxsubmit" role="form" action="<?php echo U('Administrator/rolehandle?action=edit&id='.$data_id);?>" method="post">

    <div class="form-group">
        <label class="col-sm-2 control-label"><?php echo (L("role_name")); ?></label>
        <div class="col-sm-10">
            <input name="name" type="text" class="form-control" placeholder="" required="required" value="<?php echo (htmlspecialchars($data_name)); ?>" style="width:auto;" />
            <div class="help-block"><?php echo (L("role_name_desc")); ?></div>
        </div>
    </div>
    <div class="form-group">
        <label class="col-sm-2 control-label"><?php echo (L("role_status")); ?></label>
        <div class="col-sm-10">
            <label class="radio-inline">
                <input type="radio" name="status" value="1"<?php if($data_status == 1): ?>checked="checked"<?php endif; ?>> <?php echo (L("status_normal")); ?>
            </label>
            <label class="radio-inline">
                <input type="radio" name="status" value="0"<?php if($data_status == 0): ?>checked="checked"<?php endif; ?>> <?php echo (L("status_forbidden")); ?>
            </label>
    ...
    <div class="form-group">
        <label class="col-sm-2 control-label"><?php echo (L("role_access")); ?></label>
        <div class="col-sm-10">
            <script type="text/javascript">
                $(function(){
            ...
            <?php if(is_array($data_nodes)): foreach($data_nodes as $k=>$n): if($n["name"] == 'Admin'): if(is_array($n["_child"])): foreach($n["_child"] as $kk=>$nn): if(($n["status"] == 1) AND ($nn["status"] == 1)): ?><div style="border-bottom:1px dotted #CCC;font-weight:bold;">
                                <label class="checkbox-inline">
                                    <?php if(in_array($nn['id'], $data_accesses)): ?><input type="checkbox" class="checkbox-parent" name="nodeids[]" checked="checked" value="<?php echo ($nn["id"]); ?>-<?php echo ($nn["level"]); ?>" />
                                    <?php else: ?>
                                        <input type="checkbox" class="checkbox-parent" name="nodeids[]" value="<?php echo ($nn["id"]); ?>-<?php echo ($nn["level"]); ?>" /><?php endif; ?>
                                    <?php echo ($nn["title"]); ?>
                                </label>
                            </div>
                            <div style="overflow:hidden;padding-bottom:1em;">
                                <?php if(is_array($nn["_child"])): foreach($nn["_child"] as $kkk=>$nnn): ?><div class="pull-left" style="padding-right:1em;">
                                        <?php if(($n["status"] == 1) AND ($nn["status"] == 1) AND ($nnn["status"] == 1)): ?><label class="checkbox-inline">
                                                <?php if(in_array($nnn['id'], $data_accesses)): ?><input type="checkbox" class="checkbox-child" name="nodeids[]" checked="checked" value="<?php echo ($nnn["id"]); ?>-<?php echo ($nnn["level"]); ?>" />
                                                <?php else: ?>
                                                    <input type="checkbox" class="checkbox-child" name="nodeids[]" value="<?php echo ($nnn["id"]); ?>-<?php echo ($nnn["level"]); ?>" /><?php endif; ?>
                                                <?php echo ($nnn["title"]); ?>
                                            </label><?php endif; ?>
